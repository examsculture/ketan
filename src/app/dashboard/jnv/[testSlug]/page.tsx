...
export default function JNVTestPage() {
  ...
  useEffect(() => {
    ...
    const fetchQuestions = async () => {
      const tableName = `jnv_${testSlug?.toString().replace("-", "_")}_questions`;
      const { data, error } = await supabase
        .from(tableName)
        .select("*")
        .order("id");
  ...
  return (
    <main style={{ padding: "2rem", maxWidth: "900px", margin: "0 auto" }}>
      <h1 style={{
        ...
        JNV {testSlug?.toString().replace("-", " ").toUpperCase()}
      </h1>
      {questions.length === 0 ? (
